...
class HierarchicalTetrahedraModel:
    ...
    def setup_functions(self):
        ...
        def build_mean_covariance_from_weighted_vertices(vertices, weight, scaling_modifier = 1.0, cov_modifier = 1e-9, rotation_quaternion=None):
            ...
            if rotation_quaternion is not None:
                ...
                cov = 1.0*R @ cov @ R.transpose(-1, -2) # R * Cov * R^T
    ...
    def __init__(self, sh_degree, scaling_modifier=1.0, use_cell_opacity=True, use_cell_scale=False, max_depth=4, optimizer_type="default", weight_activation_type="exp", optimizable_rotation=False):
        ...
        self.boundary_reg_axis_idx = 2
        self.boundary_reg_range = None
        self.boundary_face_indices = torch.empty((0, 3), dtype=torch.long)
        self.boundary_edge_indices = torch.empty((0, 2), dtype=torch.long)
        self.boundary_adjacent_face_pairs = torch.empty((0, 2), dtype=torch.long)
        self.boundary_vertex_indices = torch.empty((0,), dtype=torch.long)
        self.boundary_initial_vertices = torch.empty((0, 3))
    ...
    def create_from_tetra(self, tetra_data, spatial_lr_scale: float, low_scale_init: bool, training_args=None):
        ...
        if tetra_data.colors is not None:
            ...
            fused_color_sh = RGB2SH(1.0*torch.ones((self.num_root_vertices, 3), device=fused_xyz.device))
        ...
        if self.use_cell_opacity:
            # 使用单元级透明度
            init_cell_opacity = self.inverse_cell_opacity(torch.ones_like(tetra_scale) * 1.0)  # 初始透明度为0.1
    ...
    def setup_boundary_surface_regularizer(self, cameras, lod=0, eps=1e-6):
        if cameras is None or len(cameras) == 0:
            self.boundary_face_indices = torch.empty((0, 3), dtype=torch.long, device=self._xyz.device)
            self.boundary_edge_indices = torch.empty((0, 2), dtype=torch.long, device=self._xyz.device)
            self.boundary_adjacent_face_pairs = torch.empty((0, 2), dtype=torch.long, device=self._xyz.device)
            self.boundary_vertex_indices = torch.empty((0,), dtype=torch.long, device=self._xyz.device)
            self.boundary_reg_range = None
            return

        cells = self.get_cells(lod).detach().cpu().numpy().astype(np.int64)
        vertices = self.get_vertices(lod).detach().cpu().numpy().astype(np.float32)
        axis_names = sorted(
            {
                str(cam.image_path).split("/")[-1].split("\\")[-1].lower()
                for cam in cameras
            }
        )
        selected_mask = np.zeros((cells.shape[0],), dtype=bool)
        ...
        if len(axis_names) == 1:
            ...
            for cam in cameras:
                ...
                plane_coords.append(float(cam_center[axis_idx].item()))
            plane_max = max(plane_coords)
            sorted_plane_coords = sorted(plane_coords)
            if len(sorted_plane_coords) >= 2:
                plane_diffs = np.diff(np.asarray(sorted_plane_coords, dtype=np.float32))
                positive_diffs = plane_diffs[plane_diffs > eps]
                plane_spacing = float(np.median(positive_diffs)) if positive_diffs.size > 0 else float(np.median(np.abs(plane_diffs)))
            else:
                plane_spacing = 0.0
            extended_plane_max = plane_max + 2.0 * plane_spacing

            mesh_axis_min = float(vertices[:, axis_idx].min())
            tet_axis = vertices[cells][:, :, axis_idx]
            tet_min = tet_axis.min(axis=1)
            tet_max = tet_axis.max(axis=1)
            selected_mask = (tet_min <= extended_plane_max + eps) & (tet_max >= mesh_axis_min - eps)
            self.boundary_reg_axis_idx = axis_idx
            self.boundary_reg_range = (mesh_axis_min, extended_plane_max)
        else:
            for axis_name in axis_names:
                axis_idx = {"x": 0, "y": 1, "z": 2}.get(axis_name, 2)
                plane_coords = []
                for cam in cameras:
                    cam_axis_name = str(cam.image_path).split("/")[-1].split("\\")[-1].lower()
                    if cam_axis_name != axis_name:
                        continue
                    cam_center = cam.camera_center.detach()
                    plane_coords.append(float(cam_center[axis_idx].item()))
                if not plane_coords:
            ...
            self.boundary_reg_axis_idx = -1
            self.boundary_reg_range = None

        selected_cells = cells[selected_mask]
        ...
        faces = selected_cells[:, face_patterns].reshape(-1, 3)
        ...
        self.boundary_face_indices = torch.as_tensor(boundary_faces_np, dtype=torch.long, device=self._xyz.device)
        self.boundary_edge_indices = torch.as_tensor(boundary_edges_np, dtype=torch.long, device=self._xyz.device)
        self.boundary_adjacent_face_pairs = torch.as_tensor(adjacent_face_pairs_np, dtype=torch.long, device=self._xyz.device)
        self.boundary_vertex_indices = torch.as_tensor(boundary_vertices_np, dtype=torch.long, device=self._xyz.device)
        self.boundary_initial_vertices = self.get_vertices(lod).detach().clone()

    def compute_boundary_laplacian_loss(self, lod=0):
        if self.boundary_edge_indices.numel() == 0 or self.boundary_vertex_indices.numel() == 0:
            return torch.tensor(0.0, device=self._xyz.device)

        vertices = self.get_vertices(lod)
        edges = self.boundary_edge_indices
        src = edges[:, 0]
        dst = edges[:, 1]

        neighbor_sum = torch.zeros_like(vertices)
        degree = torch.zeros((vertices.shape[0], 1), dtype=vertices.dtype, device=vertices.device)
        ones = torch.ones((edges.shape[0], 1), dtype=vertices.dtype, device=vertices.device)

        neighbor_sum.index_add_(0, src, vertices[dst])
        neighbor_sum.index_add_(0, dst, vertices[src])
        degree.index_add_(0, src, ones)
        degree.index_add_(0, dst, ones)

        boundary_ids = self.boundary_vertex_indices
        boundary_degree = degree[boundary_ids].clamp_min(1.0)
        boundary_mean = neighbor_sum[boundary_ids] / boundary_degree
        return ((vertices[boundary_ids] - boundary_mean) ** 2).sum(dim=1).mean()

    def compute_boundary_displacement_laplacian_loss(self, lod=0):
        if self.boundary_edge_indices.numel() == 0 or self.boundary_vertex_indices.numel() == 0:
            return torch.tensor(0.0, device=self._xyz.device)
        if self.boundary_initial_vertices.numel() == 0:
            self.boundary_initial_vertices = self.get_vertices(lod).detach().clone()

        vertices = self.get_vertices(lod)
        initial_vertices = self.boundary_initial_vertices.to(device=vertices.device, dtype=vertices.dtype)
        displacement = vertices - initial_vertices
        edges = self.boundary_edge_indices
        src = edges[:, 0]
        dst = edges[:, 1]

        diff = displacement[src] - displacement[dst]
        return (diff**2).sum(dim=1).mean()

    def compute_boundary_dihedral_preservation_loss(self, lod=0, eps=1e-8):
        if (
            self.boundary_face_indices.numel() == 0
            or not hasattr(self, "boundary_adjacent_face_pairs")
            or self.boundary_adjacent_face_pairs.numel() == 0
        ):
            return torch.tensor(0.0, device=self._xyz.device)
        if self.boundary_initial_vertices.numel() == 0:
            self.boundary_initial_vertices = self.get_vertices(lod).detach().clone()
        ...
        def face_normals(face_vertices):
            ...
            return normals / torch.linalg.norm(normals, dim=1, keepdim=True).clamp_min(eps)

        faces = self.boundary_face_indices
        face_pairs = self.boundary_adjacent_face_pairs
        vertices = self.get_vertices(lod)
        initial_vertices = self.boundary_initial_vertices.to(device=vertices.device, dtype=vertices.dtype)

        current_normals = face_normals(vertices[faces])
        initial_normals = face_normals(initial_vertices[faces]).detach()
        current_cos = (current_normals[face_pairs[:, 0]] * current_normals[face_pairs[:, 1]]).sum(dim=1)
        initial_cos = (initial_normals[face_pairs[:, 0]] * initial_normals[face_pairs[:, 1]]).sum(dim=1)
        return ((current_cos - initial_cos) ** 2).mean()
    ...
    def training_setup(self, training_args):
        ...
        # 添加透明度相关参数
        if hasattr(self, "_cell_o") and self._cell_o is not None:
            self._cell_o.requires_grad_(False)
    ...
    def get_gs_mean_covs(self, lod):
        ...
        # 当前占据渲染训练只使用 tetra 几何本身的协方差，不再额外乘单元级 scale。
        # 保留 self._cell_scale 相关字段仅用于旧 checkpoint 兼容。
    ...
    def get_gs_mean_covs_uniform(self, lod):
        """
        Compute Gaussian mean/covariance from tetra geometry with fixed uniform weights 0.25.
        This is used by occupancy rendering to avoid learned-weight center drift on thin boundaries.
        """
        vertices = self.get_vertices(lod)
        cells = self.level_cells[lod]
        cell_vertices = vertices[cells]
        cell_weights = torch.full(
            (cell_vertices.shape[0], 4, 1),
            0.25,
        ...
        rotation_quaternion = None
        if self.optimizable_rotation and self._cell_rotation.shape[0] > 0:
            if lod == self.current_lod_depth:
                rotation_quaternion = self.rotation_activation(self._cell_rotation)

        mean, covs = self.mean_covariance_activation(
            cell_vertices,
            cell_weights,
            cell_scaling_modifier,
            cov_modifier=1e-8,
    ...
    def _collect_gaussian_export_arrays(self, lod=None, export_opacity=None):
        if PlyData is None or PlyElement is None:
            raise ImportError("plyfile is required to export Gaussian PLY.")
        if lod is None:
            lod = self.current_lod_depth
        cells = self.get_cells(lod)
        weights = self.weight_activation(self._weights)[cells].clamp(min=0.00001,max=30.0)
        mean,cov3d = self.get_gs_mean_covs_uniform(lod)
        opacity = self.get_opacities(lod)
        ...
        weighted_features_dc = torch.sum(cell_features_dc * cell_weights, dim=1) / sum_weights  # [T, 3, 1]
        weighted_features_rest = torch.sum(cell_features_rest * cell_weights, dim=1) / sum_weights  # [T, 3, C-1]
        feature_dc = weighted_features_dc # (T,4,3)
        feature_rest = weighted_features_rest # (T,4,3)
        
        xyz = mean.detach().cpu().numpy()
        normals = np.zeros_like(xyz)
        f_dc = feature_dc.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        f_rest = feature_rest.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        opacities= opacity.detach().cpu().numpy()
        if export_opacity is not None:
            opacities = np.full_like(opacities, float(export_opacity), dtype=np.float32)
        def get_scaling_rotation(Sigma_cond):
            Sigma = torch.zeros(Sigma_cond.shape[0],3,3)
            Sigma[:,0,0]=Sigma_cond[:,0]
            Sigma[:,1,0]=Sigma_cond[:,1]
            Sigma[:,0,1]=Sigma_cond[:,1]  
            Sigma[:,2,0]=Sigma_cond[:,2]
            Sigma[:,0,2]=Sigma_cond[:,2]
            Sigma[:,1,1]=Sigma_cond[:,3]
            Sigma[:,2,1]=Sigma_cond[:,4]
            Sigma[:,1,2]=Sigma_cond[:,4]   
            Sigma[:,2,2]=Sigma_cond[:,5]
            U, S, _ = torch.svd(Sigma)
            #提取旋转矩阵和缩放因子
            s = torch.sqrt(S).cuda()
            R=U.clone()
            R[:,:, 2]=U[:,:,2]*torch.sign(torch.det(U)).unsqueeze(1) # 确保右手坐标系
            # 计算四元数的各个分量
            def rotation_matrix_to_quaternion(rotation_matrix, eps=1e-6):
                ...
                mask_c1 = mask_d2 * (~mask_d0_d1)  # 使用逻辑运算符
                mask_c2 = (~mask_d2) * mask_d0_nd1  # 使用逻辑运算符
                mask_c3 = (~mask_d2) * (~mask_d0_nd1)  # 使用逻辑运算符
                ...
                q *= 0.5
                return q
            r=rotation_matrix_to_quaternion(R).cuda()
            return s,r
        sca,rot=get_scaling_rotation(cov3d)
        scale=sca.detach().cpu().numpy()
    ...
    def _write_gaussian_ply(self, output_path, arrays, cell_indices=None, apply_render_mask=True):
        l = ['x', 'y', 'z', 'nx', 'ny', 'nz']
        f_dc = arrays["f_dc"]
        f_rest = arrays["f_rest"]
        for i in range(f_dc.shape[1]):
            l.append('f_dc_{}'.format(i))
        for i in range(f_rest.shape[1]):
            l.append('f_rest_{}'.format(i))
        l.append('opacity')
        for i in range(arrays["scale"].shape[1]):
            l.append('scale_{}'.format(i))
        for i in range(arrays["rotation"].shape[1]):
            l.append('rot_{}'.format(i))
        construct_list_of_attributes=l
        dtype_full = [(attribute, 'f4') for attribute in construct_list_of_attributes]
        base_mask = np.ones(arrays["xyz"].shape[0], dtype=bool)
        if apply_render_mask and self.render_tet_mask is not None and self.render_tet_mask.numel() > 0:
            render_mask = self.render_tet_mask[..., 0].detach().cpu().numpy().astype(bool)
            base_mask &= render_mask
        if cell_indices is not None:
            subset_mask = np.zeros(arrays["xyz"].shape[0], dtype=bool)
            subset_mask[np.asarray(cell_indices, dtype=np.int64)] = True
            base_mask &= subset_mask

        xyz = arrays["xyz"][base_mask]
        normals = arrays["normals"][base_mask]
        f_dc = arrays["f_dc"][base_mask]
        f_rest = arrays["f_rest"][base_mask]
        opacities = arrays["opacities"][base_mask]
        scale = arrays["scale"][base_mask]
        rotation = arrays["rotation"][base_mask]
    ...
    def convert_gaussian(self, output_path="tetrahedra_gaussian.ply", export_opacity=None):
        """
           convert tetrahedra to gaussians
        """
        arrays = self._collect_gaussian_export_arrays(lod=self.current_lod_depth, export_opacity=export_opacity)
        self._write_gaussian_ply(output_path, arrays, cell_indices=None, apply_render_mask=True)
        return arrays["mean"], arrays["cov3d"], arrays["opacity_tensor"], arrays["weights_tensor"]

    def convert_gaussian_subset(self, output_path, cell_indices, export_opacity=None, apply_render_mask=True):
        arrays = self._collect_gaussian_export_arrays(lod=self.current_lod_depth, export_opacity=export_opacity)
        self._write_gaussian_ply(output_path, arrays, cell_indices=cell_indices, apply_render_mask=apply_render_mask)
        return arrays["mean"], arrays["cov3d"], arrays["opacity_tensor"], arrays["weights_tensor"]
    ...
    def densify_and_prune(self, viewpoint_camera, max_grad, min_opacity, extent, max_screen_size):
        ...
        def get_scaling(Sigma_cond):
            ...
            s = torch.sqrt(S).cuda()
            return s
        _,cov3d=self.get_gs_mean_covs(self.current_lod_depth)
        s= get_scaling(cov3d)
        selected_pts_mask = torch.logical_and(
            selected_pts_mask,
            torch.max(s, dim=1).values > 0.05
        )
