...
AGGREGATE_SUMMARY_NAME = "occtetheart_acdc_batch_summary.json"


def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Batch-train OccTetHeart on ACDC shift-corrected label folders. "
    ...
    parser.add_argument(
        "--output-root",
        default=None,
        help="Output root. Defaults to output/acdc_occtetheart_batch_<timestamp>.",
    )
    parser.add_argument("--train-script", default="train_tet_soft_volume_rebase_nvp.py", help="OccTetHeart training script.")
...
def main():
    ...
    output_root = Path(args.output_root) if args.output_root else repo_root / "output" / f"acdc_occtetheart_batch_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
