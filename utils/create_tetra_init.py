...
def create_tetrahedral_grid(scale, Nx, Ny, Nz):
    ...
    """
    # 体素尺寸计算
    voxel_size = np.array([2*scale[0]/Nx, 2*scale[1]/Ny, 2*scale[2]/Nz])
    
    # 生成顶点坐标（完全向量化）
    x = np.linspace(-scale[0], scale[0], Nx+1)
    y = np.linspace(-scale[1], scale[1], Ny+1)
    z = np.linspace(-scale[2], scale[2], Nz+1)
