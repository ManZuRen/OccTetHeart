...
def parse_args():
    ...
    parser.add_argument("--flip-mesh-xy", action="store_true", help="Pass --flip-mesh-xy to the clean trainer.")
...
def main():
    ...
    output_root = Path(args.output_root) if args.output_root else repo_root / "output" / f"acdc_global_clean_batch_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
